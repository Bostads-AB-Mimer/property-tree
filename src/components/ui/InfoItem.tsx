...
export function InfoItem({ icon: Icon, label, value, onClick }: InfoItemProps) {
  return (
    <div 
      ...
      onClick={onClick}
    >
      <Icon className={`h-4 w-4 text-gray-400 ${onClick ? 'group-hover:text-blue-500 transition-colors' : ''}`} />
      <div>
        <p className="text-sm text-gray-500">{label}</p>
        <p className={`font-medium ${onClick ? 'group-hover:text-blue-500 transition-colors' : ''}`}>{value}</p>
      </div>
    </div>
  );
}